...
def get_vector_algoritm(name, filename):
    if name == 'bow':
...
def get_detection_algoritm(name, tweets, vectors):
    if name == "knn":
...
def main():
    ...
    detection = get_detection_algoritm(detect_algoritm, vectorization.tweets, vectorization.vectors)
    if detection is None:
        print("Vami zadany parametr detekcniho algoritmu neexistuje!")
        print_detection_algoritms()
        exit(-4)
